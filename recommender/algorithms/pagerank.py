...
def make_graph(user_interactions: pd.DataFrame, aisle_products: pd.DataFrame, aisle_products_weight=0.5) -> nx.DiGraph:
    ...
    # Создаем пустой ориентированный граф (digraph) - ребра имеют направление.
    ...
    # ШАГ 1: Делаем связи "user -> product" и "user -> aisle"
    ...
    # ШАГ 2: Далее делаем связи "aisle -> product" (все продукты категории) - если покупатель зафиксировал
...
def start_pagerank(G: nx.DiGraph, alpha=0.85) -> dict:
    ...
    # Нам нужны только "продукты", чтобы рекомендовать их пользователю.
...
def get_top_n_pagerank(pr: dict, n=10) -> list:
    ...
    top_products = []
    for node_name, score in sorted_items[:n]:
