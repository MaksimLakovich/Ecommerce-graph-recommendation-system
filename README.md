...
1. Авторизация пользователей (login/logout)
   - Вход в систему через форму логина (Django `AuthenticationForm`).
   - Поддержка logout.
   - После успешного входа -> переход на страницу "Мои предпочтения".
   - Используется кастомная модель `AppUser`.

2. Добавление пользовательских взаимодействий
3. Получение рекомендаций (PageRank, CF, kNN)
4. Статистика и аналитика
5. Документация API (Swagger/ReDoc) будет доступна по адресу:
...
- [x] Создание сюмплов из оригинального датасета и загрузка данных в БД
- [ ] Интерфейс взаимодействия с покупателем (возможность ввода и просмотра предпочтений)
- [ ] API для предпочтений (эндпоинты для добавления предпочтений и их получения)
- [ ] Алгоритмы PageRank / CF / kNN
- [ ] Интерфейс взаимодействия с покупателем (кнопка для получения рекомендаций / веб-страница для отображения рекомендаций пользователю)
- [ ] API для рекомендаций (эндпоинты для получения рекомендаций на основе покупок и предпочтений)
...
- [ ] Тестирование (написание тестов для проверки корректности работы алгоритмов и системы в целом0
- [ ] CI/CD для продакшн-сервера

#### Будущие доработки (развитие системы):
- [ ] Добавить авторизацию в системе по email + изменить в "UserPreference(models.Model)" заполнение "user = models.ForeignKey(AppUser, ...)" на user.request
- [ ] Добавить права доступов (отображать статистику, рекомендации, предпочтения только авторизованному пользователю)
